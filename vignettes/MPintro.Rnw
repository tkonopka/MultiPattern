...
config.groups = list("clust" = grep("clust", config.names, val=T),
                     "pca" = grep("PC", config.names, inv=T, val=T))
@
...
MPplotmap(mp6.metamap, col=config.groups, Rcssclass="meta", label=TRUE)
@
...
mp6.clust = matrix(NA, ncol=length(mp6.reps), nrow=nrow(D6))
rownames(mp6.clust) = rownames(D6)
colnames(mp6.clust) = names(mp6.reps)
for (i in names(mp6.reps)) {
  temp = pamclusters(mp6.sims[[mp6.reps[i]]], prefix=i)
  mp6.clust[names(temp), i] = temp	
  rm(temp, i)
}
@
...
mp6.multilabs = sort(table(mp6.multilabels), decreasing=T)
mp6.multilabs
@
...
<<plotMU, eval=FALSE>>=
par(mfrow=c(2,4))
for (uu in names(mp6.multilabs)[1:min(8, length(mp6.multilabs))]) {
...
\noindent This should print a message outlining each setting component (omitted here for brevity). For example, {\tt subsample.R} determines the number of bootstrap repetitions performed by function {\tt MPgetAverageMetaDistance}. 

Values for analysis settings can be changed either using function {\tt MPchangeSettings} or via direct manipulation.

<<update3>>=
mp.temp = mp6
mp.temp$settings$subsample.R
mp.temp = MPchangeSettings(mp.temp, list(subsample.R=100))
mp.temp$settings$subsample.R
mp.temp$settings$subsample.R = 150
mp.temp$settings$subsample.R
@ 

\noindent The object {\tt mp.temp} is a scratch object. The rest of the block demonstrates two ways to change a setting value. The former has the advantage of providing a warning while attempting to modify a non-core settings.

<<update4>>=
mp.temp$settings$subsample.R
mp.temp = MPchangeSettings(mp.temp, list(sbsmpl.R=75))
mp.temp$settings[c("subsample.R", "sbsmpl.R")]
@ 
...
A core step of a multi-pattern analysis is defining a collection of configurations. Each configuration is a traditional approach to pattern analysis based on a distance or dissimilarity function. The package provides a factory function {\tt MPdistFactory} that creates appropriate distance functions. This factory can produce some straight-forward as well as some non-traditional functions. 
...
## helper function - test that two functions produce same output on a dataset
...
<<newconf2, eval=TRUE>>=
mp6.custom = MPnew(rownames(D6), list("D6" = D6[,1:2]))
mp6.custom
@ 
...
<<newconf3>>=
mp6.custom = MPaddConfig(mp6.custom, "euclidean", data="D6", dist.fun=dist.euclidean)
mp6.custom = MPaddConfig(mp6.custom, "canberra",  data="D6", dist.fun=dist.canberra)
mp6.custom = MPaddConfig(mp6.custom, "reg",       data="D6", dist.fun=dist.regularized)
mp6.custom
@ 
...
D6b = data.frame(D6[,1:2], 
    D3=(D6[,1]/10)+(D6[,2]/8), 
    D4=(D6[,1]/6)-(D6[,2]/9))
mp6.fam = MPnew(rownames(D6), list("D6b"=D6b))
mp6.fam
@ 
...
<<family2>>=
mp6.fam = MPeasyConfig(mp6.fam, data="D6b", type="pca")
mp6.fam
names(mp6.fam$configs)
@ 
...
<<family4>>=
mp6.fam = MPeasyConfig(mp6.fam, data="D6b", type="reg")
mp6.fam
names(mp6.fam$configs)
@ 
